...
if __name__=='__main__':
    ...
    #analiza mezczyzn
    m='m22_10.15.cut_500'   #nazwa pliku mezczyzny. Pliki znajdują się w folderze patient
    w,who=tablica(m)
    z=list()
    z=[float(w[i]) for i in range(len(w)-1)]
    print("Tachograf znajduje sie w folderze ./wyniki/m/plot_RR")
    d_z.Ploty_zapisy(who,'m',z)
    ...
    plt.savefig("./wyniki/m/"+who+"/coef_"+who+'.jpg',dpi=80)   
    print("Wykres z 5 pierwszymi współczynnikami znajduje się w: ./wyniki/m/ nazwa_pacjenta /")
    plt.show()
    ...
    for i in range(len(y)):
        ...
        plik.write('\n')
    plik.close()
    print("Wszystkie współczynniki AR znajdują się w folderze: ./wyniki/m/ nazwa_pacjenta /")
    # zmiana tab na array
    ...
    plt.savefig('./wyniki/m/fazy_plot/'+who+'_plot.jpg',dpi=80)         #tu 
    plt.show()
    tt=tt+1
    
    #d_z.Ilosc_faz_zapis(tab_ilosc_faz,'./wyniki/m/')           #tu
    d_z.Fazy_zapis(tab_fazy,'./wyniki/m/')
    
    print("Cykle snu znajdują się w folderze ./wyniki/m/fazy_plot/")